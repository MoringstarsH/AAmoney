...
function App() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      </div>rn null; return (<div key={m.id} className="flex justify之间 items-center text-xs"><span className="text-slate-500 font-medium">{m.name}</span><span className={`font-bold ${isPos ? 'text-teal-500' : 'text-rose-500'}`}>{isPos ? '+' : ''}{formatMoney(bal)}</span></div>); })}</div><div className="mt-8 text-[10px] text-slate-300 font-mono">Generated by LumiSplit</div></div></div>
